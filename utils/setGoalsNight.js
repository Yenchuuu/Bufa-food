...
async function crontab() {
  try {
    ...
    for (let userInfo of users) {
      await db.execute('UPDATE `user_goal` SET goal_calories = ?, goal_carbs = ?, goal_protein = ?, goal_fat = ?, date = ? WHERE user_id = ?;', [userInfo.goal_calories, userInfo.goal_carbs, userInfo.goal_protein, userInfo.goal_fat, date, userInfo.user_id])
    }
