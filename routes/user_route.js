...
const { signUp, nativeSignIn, fbSignIn, setUserTarget, getUserProfile, updateUserProfile, uploadUserImage, deleteUserImage, updateUserBodyInfo, updateNutritionTarget, getUserPreference, getDailyGoal, setDailyGoal } = require('../controller/user_controller')
...
router.route('/user/profile/image/:id').patch(authentication(), cpUpload, wrapAsync(uploadUserImage))
router.route('/user/profile/image/:id').delete(authentication(), wrapAsync(deleteUserImage))
...
router.route('/start-scheduler').post(setDailyGoal)
